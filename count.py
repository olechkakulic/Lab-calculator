    ...
    def get_T_prev(n, n_pod, angle, n_prev, n_next):
        ...
        if polarisation == 'TE':
            ...
            def Ei(ni):
                Ei = ni ** 2
                return Ei
            r_prev = (Ei(n) * kzi(n_prev) - Ei(n_prev) * kzi(n)) /\
                     (Ei(n) * kzi(n_prev) + Ei(n_prev) * kzi(n))
            r = (Ei(n_next) * kzi(n) - Ei(n) * kzi(n_next)) /\
                (Ei(n_next) * kzi(n) + Ei(n) * kzi(n_next))
            t = 2 * Ei(n) * kzi(n_prev) /\
                ( Ei(n) * kzi(n_prev) + Ei(n_prev) * kzi(n))
            t_prev = 2 * Ei(n_next) * kzi(n) / \
                     (Ei(n_next) * kzi(n) + Ei(n) * kzi(n_next))
